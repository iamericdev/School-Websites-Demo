...
export const SchoolSwitcher: React.FC<SchoolSwitcherProps> = ({ current, onSelect }) => {
  const [isOpen, setIsOpen] = useState(false);

  const schools = [
    { id: 'school1', name: 'American University (School 1)' },
    { id: 'school2', name: 'JCT Institutions (School 2)' }
  ] as const;
      ...
      <div className="relative">
        ...
        {isOpen && (
          <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="p-2">
              <p className="px-3 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Select Design</p>
              {schools.map((school) => (
                <button
                  key={school.id}
                  onClick={() => {
